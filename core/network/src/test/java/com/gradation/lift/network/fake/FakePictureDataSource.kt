...
import com.gradation.lift.network.data.TestDtoDataGenerator.Picture.userProfilePictureResponseDto
...
class FakePictureDataSource(private val testReturnState: TestReturnState = TestReturnState.Success) :
    ...
    override suspend fun getUserProfilePicture(): Flow<APIResult<List<UserProfilePicture>>> =flow {
        when (testReturnState) {
            ...
            TestReturnState.Success -> emit(APIResult.Success(data = userProfilePictureResponseDto.toUserProfilePicture()))
        }
    }
