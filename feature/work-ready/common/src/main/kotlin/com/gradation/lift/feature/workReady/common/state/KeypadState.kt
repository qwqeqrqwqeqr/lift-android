...
class KeypadState {
    ...
    val selectedRoutineIndex: MutableStateFlow<Int?> = MutableStateFlow(null)
    val selectedWorkSetIndex: MutableStateFlow<Int> = MutableStateFlow(0)
    val selectedWorkSet: MutableStateFlow<WorkRoutineWorkSet> =
        MutableStateFlow(WorkRoutineWorkSet())

    val init: (Int?, Int, WorkRoutineWorkSet) -> Unit =
        { routineIndex, workSetIndex, workSet ->
            onKeypadEvent(
                KeypadEvent.Init(
                    routineIndex,
                    workSetIndex,
    ...
    private fun onKeypadEvent(keypadEvent: KeypadEvent) {
        when (keypadEvent) {
            ...
            is KeypadEvent.Init -> {
                selectedRoutineIndex.value = keypadEvent.routineIndex
                selectedWorkSetIndex.value = keypadEvent.workSetIndex
