...
internal class CompleteDetailViewModel @Inject constructor(
    ...
    val createHistory: (WorkRestTime, List<CreateHistoryRoutine>) -> Unit =
        { workRestTime, historyRoutineList ->
            viewModelScope.launch {
                createHistoryUseCase(
                    ...
                    when (it) {
                        is DataState.Fail -> createWorkHistoryState.value = CreateWorkHistoryState.Fail(message = it.message)

                        is DataState.Success -> createWorkHistoryState.value = CreateWorkHistoryState.Success
